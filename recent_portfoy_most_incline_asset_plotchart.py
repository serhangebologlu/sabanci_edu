...
if dfMarkets.count() > 0 :
    ...
    # Market verileri içinden müşteri varlıklarından son 24 saatte fiyatı en çok artan bulunuyor
    for row in customer_asset_list:
        ...
        if foundRow is not None:
            if biggestPrice < foundRow.change_24h:
                biggestPrice = foundRow.change_24h
                biggestAssetId = foundRow.base_asset
                biggestExchangeId = foundRow.exchange_id
    # Market verileri içinden müşteri varlıklarından son 24 saatte fiyatı en çok artan bulunuyor  
    
    # Bulunan varlığın son 24 saatteki fiyatları alınıyor
    dfMarkets = dfMarkets.filter(col("exchange_id") == biggestExchangeId).filter(col("base_asset") == biggestAssetId)
