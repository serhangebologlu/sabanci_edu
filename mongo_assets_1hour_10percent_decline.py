...
if df.count() > 0:
    ...
    # Son 1 saatte düşen veriler filtreleniyor
    dfHour = df.filter("change_1h < 0")
    # Son 1 saatte düşen veriler filtreleniyor
    
    # %10 oranında düşen varlıklar bulunuyor
    dfHour = dfHour.withColumn("price_1h_ago", - col("change_1h") + col("price"))
    dfHour = dfHour.filter(-col("change_1h")/col("price_1h_ago")>0.1)
    dfHour.select("asset_id", "change_1h", "price", "updated_at", "price_1h_ago").show()
    # %10 oranında düşen varlıklar bulunuyor
